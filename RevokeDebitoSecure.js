...
if(autenticacion != ""){
	...
	print ("Pre revoke debit SM: " + new ByteString("8C E8 00 00",HEX).concat(isoIn).concat(TLV87).concat(new ByteString("8E 04", HEX)).concat(MACcredit));
	var resp = card.plainApdu(new ByteString("8C E8 00 00",HEX).concat(isoIn).concat(TLV87).concat(new ByteString("8E 04", HEX)).concat(MACcredit));
	print ("Respuesta revoke debit SM" + resp);
	print("Código SW: " + card.SW.toString(16));
    //Get Response
	print("");
	...
	if(autenticacion != ""){
		var respuestaConc = TLVDebR.concat(autenticacion.left(4));
		var MacComprobarRelleno = respuestaConc.pad(Crypto.ISO9797_METHOD_2, true);
