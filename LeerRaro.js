...
debitKey         = new ByteString("80 81 82 83 84 85 86 87", HEX); //Kd
...
function debitSM(){
	amount = new ByteString("00 04 E2", HEX); //1250 -> 12,5 euros
	newATREF = ATREF.add(1);
	
	debitChecksum = new ByteString("E6", HEX).concat(amount).concat(TTREF_D).concat(newATREF).concat(new ByteString("00 00", HEX));
	deskey.setComponent(Key.DES, debitKey);
	MAC = crypto.encrypt(deskey, Crypto.DES_CBC, debitChecksum, IvAccount);
	
	deskey.setComponent(Key.DES, Ks);

    debitData = MAC.right(8).left(4).concat(amount).concat(TTREF_D);

    encriptedData = crypto.encrypt(deskey, Crypto.DES_CBC, debitData.pad(Crypto.ISO9797_METHOD_2), seqNumber);

    paddingInd = "0" + (debitData.pad(Crypto.ISO9797_METHOD_2).length - debitData.length).toString(16);
    ...
    debitAccApdu = new ByteString("8C E6 00 00", HEX);
    commandHeaderTLV =  new ByteString("89 04", HEX).concat(debitAccApdu);  
    ...
    print("APDU: " + debitAccApdu.concat(new ByteString(P3.toString(16), HEX)).concat(secureData).concat(new ByteString("8E 04", HEX)).concat(MAC));
    card.plainApdu(debitAccApdu.concat(new ByteString(P3.toString(16), HEX)).concat(secureData).concat(new ByteString("8E 04", HEX)).concat(MAC));
    print("Código SW: " + card.SW.toString(16));
}

function verifyDebitSM(){
  getResponseApdu  = "80 C0 00 00".concat(responseLength);
  response = card.plainApdu(new ByteString(getResponseApdu, HEX));
  print ("resp: " + response);
  swStatus = response.left(4).right(2);
  intents = swStatus.right(1)
  print (" -x-x-x-x-x-x- Account Status after Debit: " +swStatus + " -x-x-x-x-x-x- ");
   //----------------------------------------------- Verify SM MAC  
  if (response != "") {
    print ("TRACE --- Debit - Checking Secure Debit Response");
    ...
    print ("Received MAC Encripted with Ks - ok");
//-----------------------------------------------------
    print ("TRACE --- Debit Account with Secure Messaging Done Correctly");
  }
}
