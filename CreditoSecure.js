...
if(autenticacion != ""){
	...
	var TLVCredito = new ByteString("89 04 8C E2 00 00",HEX);
	TLVCred = TLVCredito.concat(TLV87);
	...
	print ("Pre credit SM: " + new ByteString("8C E2 00 00",HEX).concat(isoIn).concat(TLV87).concat(new ByteString("8E 04", HEX)).concat(MACcredit));
	var resp = card.plainApdu(new ByteString("8C E2 00 00",HEX).concat(isoIn).concat(TLV87).concat(new ByteString("8E 04", HEX)).concat(MACcredit));
	print ("Respuesta credit SM" + resp);
	print("Código SW: " + card.SW.toString(16));
    //Get Response
	print("");
	...
	if(autenticacion != ""){
		var respuestaConc = TLVCredito.concat(autenticacion.left(4));
		var MacComprobarRelleno = respuestaConc.pad(Crypto.ISO9797_METHOD_2, true);
...
//  llamada a otro fichero por falta de tiempo, lo siento
