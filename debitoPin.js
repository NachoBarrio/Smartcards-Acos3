...
//lanzar debit 2240 unidades
var retiro = new ByteString("00 08 C0",HEX);
var ATREF2 = ATREF.add(1);
var MacCredit = new ByteString("E6",HEX).concat(retiro).concat(TTREFD).concat(ATREF2).concat(new ByteString("00 00",HEX));
...
resp = card.plainApdu(new ByteString("80 E6 01 00 0B",HEX).concat(Mac4Credit).concat(retiro).concat(TTREFC));
print("Código SW: " + card.SW.toString(16));
...
print("inquire " + resp);
resp = card.plainApdu(new ByteString("80 C0 00 00 08", HEX));
print ("Respuesta al 2 inquire " + resp);
